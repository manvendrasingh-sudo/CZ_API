...
$host   = $header_name;
// Host -- We are getting host entry to check that request is made from local host only and no public access is allowed  
...
$url = "http://127.0.0.1/apps/addlead.php";
$ch  = curl_init();
...
foreach ($param as $i => $v) {
    if (is_array($v)) {
        ...
        if ($resFormat) {
            $final .= (empty($final) ? '"RES' . $i . '": {"mobile": "' . $mobile . '","result": "' . $result . '"}' : ',"RES' . $i . '": {"mobile": "' . $mobile . '","result": "' . $result . '"}');
        } else {

...
class Reportoutput extends DBconnection implements OutputInterface
{
    ...
    public function setReportFields($reportObj, $config, $filerObj, $report_name, $current_month, $dwnl_reports_flag, $report_user_name,$table_name,$page, $limit)
    {
        ...
        if ($this->report_name === 'agent_session_report' || $this->report_name === "agent_login_report" || $this->report_name === "agent_summary_report"){
        ...
        $timeZoneVariablesArr      = $filerObj->getTimeZoneVariables();
        $defTZ                     = $timeZoneVariablesArr[0];
        $timeDiffCamp              = $timeZoneVariablesArr[1];
        $this->reg_min_timestamp = "unix_timestamp(convert_tz('$strmintimestamp',if('$defTZ'= 1,timeDiff,'$timeDiffCamp'),timeDiff))";
        $this->reg_max_timestamp = "unix_timestamp(convert_tz('$strmaxtimestamp',if('$defTZ'= 1,timeDiff,'$timeDiffCamp'),timeDiff))";
    ...
    public function finalReport()
    {
        ...
        if ($this->report_name == "agent_break_report") {
            ...
            $betweenDates = rtrim($this->filterval, "and");
            $query = "select c.agent_name,c.agent_id,c.agent_identity,c.campaign_name,ifnull(d.total_call,0),ifnull(d.answd_prev,0),ifnull(d.answd_pro,0),ifnull(d.answd_pred,0),ifnull(d.disp_set,0),ifnull(d.disp_not_set,0),ifnull(d.calls_held,0),ifnull(HOUR_MINUTES(d.Acdtime),'00:00:00') as talk_time,ifnull(HOUR_MINUTES(d.Acdtime/d.total_call),'00:00:00') as avg_talk_time,ifnull(HOUR_MINUTES(d.preview_talk_time),'00:00:00'),ifnull(HOUR_MINUTES(if(d.ring_time!='0',(if((d.Acdtime -(d.ring_time + d.hold_time)) > 0,(d.Acdtime -(d.ring_time + d.hold_time)),(d.Acdtime - d.hold_time))),(d.Acdtime - d.hold_time))),'00:00:00') as actual_talk_time,ifnull(HOUR_MINUTES(d.ring_time),'00:00:00'),ifnull(HOUR_MINUTES(d.ring_time/d.total_call),'00:00:00') as avg_ring_time,ifnull(HOUR_MINUTES(d.hold_time),'00:00:00'),ifnull(HOUR_MINUTES(d.hold_time/d.total_call),'00:00:00') as avg_hold_time,ifnull(HOUR_MINUTES(d.total_call_duration),'00:00:00') as call_handled_time,ifnull(HOUR_MINUTES(d.total_call_duration/d.total_call),'00:00:00') as avg_call_handled_duration,ifnull(d.wrap_count,0),ifnull(HOUR_MINUTES(d.Acwtime),'00:00:00') as wrapup_time,ifnull(HOUR_MINUTES(d.Acwtime/d.total_call),'00:00:00') as avg_wrapup_time,ifnull(HOUR_MINUTES(c.total_login_time),'00:00:00'),ifnull(HOUR_MINUTES(c.total_login_time - ifnull(d.total_call_duration,0) - c.pauseDuration - c.conf_duration),'00:00:00') as idle_time,ifnull(HOUR_MINUTES(c.pauseDuration),'00:00:00'),ifnull(HOUR_MINUTES(c.preview_pause_duration),'00:00:00'),ifnull(HOUR_MINUTES(c.ready_time),'00:00:00'),ifnull(HOUR_MINUTES(c.total_login_time-c.pauseDuration-c.ready_time),'00:00:00') as staff_time,ifnull(HOUR_MINUTES(c.conf_duration),'00:00:00'),ifnull(HOUR_MINUTES(c.total_preview_time),'00:00:00'),ifnull(HOUR_MINUTES(if(c.total_preview_time - ifnull(d.preview_talk_time,0) - c.preview_pause_duration > 0,c.total_preview_time - ifnull(d.preview_talk_time,0) - c.preview_pause_duration,0)),'00:00:00') as  pre_idle_time FROM (SELECT agent_name,agent_id,agent_identity,campaign_id,campaign_name,sum(if((agent_state='BREAK_N_BACK' or agent_state='BREAK') and break_type not in('JoinedConference'),if(call_end_date_time='0000-00-00 00:00:00',unix_timestamp(),if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time)))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as pauseDuration,sum(if((agent_state='LOGIN_N_LOGOUT' or agent_state='LOGIN'),if(call_end_date_time='0000-00-00 00:00:00',unix_timestamp(),if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time)))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as total_login_time,sum(if(agent_state='LOGIN_N_LOGOUT',if(entrytime+ready_time>$reg_max_timestamp,$reg_max_timestamp,if(entrytime+ready_time<$reg_min_timestamp,$reg_min_timestamp,entrytime+ready_time))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as ready_time,sum(if((agent_state='BREAK_N_BACK' or agent_state='BREAK') and break_type ='JoinedConference',if(call_end_date_time='0000-00-00 00:00:00',unix_timestamp(),if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time)))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as conf_duration,sum(if((agent_state='PREVIEW' or agent_state='PREVIEW_N_PROGRESSIVE' or agent_state='PREVIEW_N_PREDICTIVE'),if(call_end_date_time='0000-00-00 00:00:00',unix_timestamp(),if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time)))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as total_preview_time,sum(if((agent_state='BREAK_N_BACK' or agent_state='BREAK') and dailer_mode='PREVIEW' and break_type not in('JoinedConference'),if(call_end_date_time='0000-00-00 00:00:00',unix_timestamp(),if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time)))-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime),0)) as preview_pause_duration FROM  agent_state_analysis_$this->table_name as a where $this->agentBetween agent_state IN ('LOGIN_N_LOGOUT','BREAK_N_BACK','LOGIN','BREAK','PREVIEW','PREVIEW_N_PROGRESSIVE','PREVIEW_N_PREDICTIVE') $this->agent_id $this->campaign_id GROUP BY agent_id,campaign_id ORDER BY agent_id) as c left join (SELECT a.campaign_id,agent_id,count(*) as total_call,sum(if((call_status_disposition='answered' or call_status_disposition='transfer') and (dialer_type='PREVIEW'),1,0)) as answd_prev,sum(if((call_status_disposition='answered' or call_status_disposition='transfer') and (dialer_type='PROGRESSIVE'),1,0)) as answd_pro,sum(if((call_status_disposition='answered' or call_status_disposition='transfer') and (dialer_type='PREDICTIVE'),1,0)) as answd_pred,sum(if(cust_disposition<>'' and cust_disposition<>'None',1,0)) as disp_set,sum(if(cust_disposition='' or cust_disposition='None',1,0)) as disp_not_set,sum(if(wrapup_time!='',1,0)) as wrap_count,sum(if(unix_timestamp(call_end_date_time)!=0,if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time))-if(entrytime<$reg_min_timestamp,if(unix_timestamp(call_end_date_time) < $reg_min_timestamp,unix_timestamp(call_end_date_time),$reg_min_timestamp),entrytime),0)) as Acdtime,sum(if(entrytime+call_duration <$reg_max_timestamp,(if(entrytime+wrapup_time+call_duration>$reg_max_timestamp,$reg_max_timestamp,entrytime+wrapup_time+call_duration)-if(entrytime+call_duration<$reg_min_timestamp,$reg_min_timestamp,entrytime+call_duration)),0)) as Acwtime,sum(if(entrytime+wrapup_time+call_duration>$reg_max_timestamp,$reg_max_timestamp,entrytime+call_duration+wrapup_time)-if(entrytime<$reg_min_timestamp,$reg_min_timestamp,entrytime)) as total_call_duration,sum(if(dialer_type='PREVIEW' || czreversedial='1',if(ringend_time!='0000-00-00 00:00:00',if((if(unix_timestamp(ringend_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(ringend_time))-if(unix_timestamp(ringstart_time)<$reg_min_timestamp,if(unix_timestamp(ringend_time) < $reg_min_timestamp,unix_timestamp(ringend_time),$reg_min_timestamp),unix_timestamp(ringstart_time))) > 0 and (if(unix_timestamp(ringend_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(ringend_time))-if(unix_timestamp(ringstart_time)<$reg_min_timestamp,if(unix_timestamp(ringend_time) < $reg_min_timestamp,unix_timestamp(ringend_time),$reg_min_timestamp),unix_timestamp(ringstart_time))) <= call_duration ,if(unix_timestamp(ringend_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(ringend_time))-if(unix_timestamp(ringstart_time)<$reg_min_timestamp,if(unix_timestamp(ringend_time) < $reg_min_timestamp,unix_timestamp(ringend_time),$reg_min_timestamp),unix_timestamp(ringstart_time)),if((if(unix_timestamp(ringend_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(ringend_time))-if(unix_timestamp(ringstart_time)<$reg_min_timestamp,if(unix_timestamp(ringend_time) < $reg_min_timestamp,unix_timestamp(ringend_time),$reg_min_timestamp),unix_timestamp(ringstart_time))) > 0 and (if(unix_timestamp(ringend_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(ringend_time))-if(unix_timestamp(ringstart_time)<$reg_min_timestamp,if(unix_timestamp(ringend_time) < $reg_min_timestamp,unix_timestamp(ringend_time),$reg_min_timestamp),unix_timestamp(ringstart_time))) > call_duration ,call_duration,'0')),'0'),0)) as ring_time,sum(if(hold_time!=0,if(entrytime+hold_time>$reg_max_timestamp,$reg_max_timestamp,entrytime+hold_time)-if(entrytime<$reg_min_timestamp,(if(unix_timestamp(call_end_date_time)<$reg_min_timestamp,(entrytime+hold_time),(entrytime + hold_time))),entrytime),0)) as hold_time,sum(if(dialer_type='PREVIEW' and ((entrytime+call_duration) >=$reg_min_timestamp and (entrytime+call_duration)<$reg_max_timestamp),hold_time,0)) as preview_hold_time,sum(if(dialer_type='PREVIEW' and unix_timestamp(call_end_date_time)!=0,if(unix_timestamp(call_end_date_time)>$reg_max_timestamp,$reg_max_timestamp,unix_timestamp(call_end_date_time))-if(entrytime<$reg_min_timestamp,if(unix_timestamp(call_end_date_time) < $reg_min_timestamp,unix_timestamp(call_end_date_time),$reg_min_timestamp),entrytime),0)) as preview_talk_time,sum(if(dialer_type='PREVIEW',if(entrytime+wrapup_time+call_duration>$reg_max_timestamp,$reg_max_timestamp,entrytime+wrapup_time+call_duration)-if(entrytime+call_duration<$reg_min_timestamp,$reg_min_timestamp,entrytime+call_duration),0)) as preview_wrap_time,sum(if(hold_time,1,0)) as calls_held,sum(if(dialer_type='PREVIEW' || czreversedial='1',if((unix_timestamp(ringend_time) - unix_timestamp(ringstart_time))>0,unix_timestamp(ringend_time) - unix_timestamp(ringstart_time),0),0)) as ring_duration FROM $this->table_name as a inner join campaign as b on a.campaign_id=b.campaign_id where agent_id!='' and b.dialerType!='AUTODIAL' and ((call_status_disposition!='supervised_transfer') or isnull(call_status_disposition)) and $betweenDates $this->campaignBetween $this->agent_id $this->campaign_id GROUP BY agent_id,a.campaign_id ORDER BY agent_id) as d on c.agent_id = d.agent_id and c.campaign_id=d.campaign_id";
        ...
        }else if ($this->report_name === "detail_call_report") {
            $where = "WHERE ". $this->filterval. " " .$this->reportArr[$this->report_name]['where'];
        } else {
    ...
    public function fetchQueryRecords($query, $page, $limit)
{

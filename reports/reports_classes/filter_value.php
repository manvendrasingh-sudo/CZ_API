...
public function setCampaignId($campaign_id, $magic_call,$ivr_report) {
        if ($magic_call == 1 && $campaign_id != "" && $ivr_report !== "1") {
            $this->campaign_id = " a." . ($magic_call == 1 ? "department_id" : "campaign_id") . "='$campaign_id' and";
        } else if(!empty($campaign_id) && $campaign_id != "" && $ivr_report !== "1") {
            $this->campaign_id = " a.campaign_id='$campaign_id' and";
        } else if(!empty($campaign_id) && $campaign_id != "" && $ivr_report == "1") { //Gaurav jain
            $this->campaign_id = " a.CampaignTransfer_id='$campaign_id' and";
        } else {
            $this->campaign_id = "";
        }
}
...
public function setFilterCondition($current_date,$startdate,$enddate,$starttime,$endtime,$startduration,$endduration,$campaign_id,$agent_id,$list_id,$disposition,$campaign_type,$dialer_type,$cust_disposition,$customer_ph,$unique_calls,$magic_call,$ivr_node_name,$did,$ivr_report,$permissible_ivr){ //add ivr node name and did Gaurav jain
        ...
        $this->setCampaignId($campaign_id,$magic_call,$ivr_report);  //Gaurav jain
...
public function setDID($did) {
        $this->did = (!empty($did)) ? " a.did = '$did' and" : "";
}

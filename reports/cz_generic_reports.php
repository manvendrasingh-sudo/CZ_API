...
// Validate token Data.
(empty($role_id) || empty($user_id)) &&  $user_id!=1 && errorResponse("Unauthorizedfff Access", 401, "Invalid user or role in token.");
...
$validReportsArr = [
    ...
    "detail_call_report",
    "magic_call_report",
    "ivr_detail_call_report" //Gaurav jain edit
];
...
function validateReqData($validReportsArr,$param){
    ...
    global $maximum_days_rep,$role_id,$user_id,$conn,$redis;
    ...
    switch ($report_name) {
        ...
        //if did and iver node name in req but its value is null  Gaurav jain start
    ...
    return compact (
        ...
        "list_name", "campaign_type" , "user_name", "did", "ivr_node_name","page","limit"
    );
}
...
$permissible_ivr         = ""; //Gaurav jain

// Checking the Validation and get the Permissionable Data
$is_valid_permissions = check_validation($redis, $role_id, $user_id, $report_name, $camp_name, $department_name, $config_data, $conn);
...
$id = array_search(strtolower($tableDetails['value_name']), $dataArr);
if(empty($id) || $id == ""){
...
$report     = new Reportoutput($db,$config_data);

$timeZonVal = campaignTimeZone($conn,$camp_name);
$filterObj->setTimeZoneVariables($timeZonVal);

// Getting the Permissible Ivr for all the Ivr related reports // Edited Gaurav Jain.
if($ivr_report == "1") {
    $existsInReddis = false;
    if($redis->get("IVR#TVT#{$role_id}#TVT#{$user_id}")){
        $existsInReddis = true;
        $permissible_ivr = $redis->get("IVR#TVT#{$role_id}#TVT#{$user_id}");
    } else {
        $permissible_ivr = getUserPermissionsFromDB($conn, ["select_val"=>"permissible_ivr"], $role_id, $user_id);
    }

    if(!empty($permissible_ivr)){
        if(!$existsInReddis){
            $redis->set("IVR#TVT#{$role_id}#TVT#{$user_id}", $permissible_ivr);
        }
    } else {
        errorResponse("Some Error Occured, try again after some time.", 500, "Unable to get the Permissible IVR | Value of Permissible IVR : $permissible_ivr");
    }
}

$filterObj->setFilterCondition($current_date, $startdate, $enddate, $starttime, $endtime, $startduration, $endduration, $id, $agent_id, $list_id, $disposition, $campaign_type, $dialer_type, $cust_disposition, $customer_ph, $unique_calls, $magic_call, $ivr_node_name, $did , $ivr_report, $permissible_ivr,$camp_name); //Gaurav jain edit did and ivr node name add
...
function check_validation($redis, $role_id, $user_id, $report_name, $camp_name, $department_name, $config_data, $conn) {
    ...
    $config = $settings[$report_name] ?? $settings["default"];
...
function getUserPermissionsFromDB($conn, $config, $role_id, $user_id) {
    ...
    $result = mysqli_fetch_object(mysqli_query($conn, "SELECT system_roles.{$config['select_val']} FROM admin LEFT JOIN system_roles ON admin.role_ID = system_roles.role_id WHERE admin.role_ID = $role_id AND admin.user_id = $user_id"));
     $select_val = $config['select_val'];
     return $result ? $result->$select_val : null;
}


function checkUserPermissions($conn, $redis, $config, $role_id, $user_id, $camp_name, $department_name,$ivr_node_name="") {
  #try{
    ...
    $condition_value = $config['table_name'] === "department" ? $department_name : ($config['table_name'] === "ivr_node" ? $ivr_node_name :$camp_name); //Gaurav jain
    $req_permissions = $redis->get($config['redis_key']) ?: getUserPermissionsFromDB($conn, $config, $role_id, $user_id);
    //$req_permissions = getUserPermissionsFromDB($conn, $config, $role_id, $user_id);
    ...
    if ($req_permissions) {
        ...
        if ($req_permissions) { //Gaurav jain 'not' condition add
 ...
 #}catch (Throwable $err){
	#prepare_log_report("checkUserPermissions() Query Error: ", $err, 3);
 #}
}
...
function prepare_log_report($event, $data, $local_log_depth = 0) {
    ...
    if($local_log_depth == 0 && ($config_log_depth == 0 || $config_log_depth == 1 || $config_log_depth == 2 || $config_log_depth == 3)){
        $logData = date("Y-m-d H:i:s") . " [$reqUid] :: [$event] :: " . (is_array($data) || is_object($data) ? json_encode($data, true) : $data) . "\n\n";

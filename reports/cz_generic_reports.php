...
function validateReqData($validReportsArr,$param){
    global $maximum_days_rep,$role_id,$user_id,$conn;
    ...
    switch ($report_name) {
        ...
        case "ivr_detail_call_report":
            ...
            // $is_valid_permissions = checkUserPermissions($conn, "", ["table_name"=>"ivr_node","search_val"=>"ivr_node_id","cond_col"=>"ivr_node_name","select_val"=>"permissible_ivr"],$role_id, $user_id,"", "",$ivr_node_name);

            if(isset($param['ivr_node_name']) && !empty($param['ivr_node_name'])){
                $is_valid_permissions = checkUserPermissions($conn, "", ["table_name"=>"ivr_node","search_val"=>"ivr_node_id","cond_col"=>"ivr_node_name","select_val"=>"permissible_ivr"],$role_id, $user_id,"", "",$ivr_node_name);

                // var_dump($is_valid_permissions);
    ...
    return compact (
        ...
        "list_name", "campaign_type" , "user_name","did","ivr_node_name",
        "page", "limit"
    );
}
...
function checkUserPermissions($conn, $redis, $config, $role_id, $user_id, $camp_name, $department_name,$ivr_node_name="") {
    ...
    // if($config['table_name'] !== "ivr_node"){
    $req_permissions = $redis->get($config['redis_key']) ?: getUserPermissionsFromDB($conn, $config, $role_id, $user_id);
    // } else {
    //     $req_permissions = getUserPermissionsFromDB($conn, $config, $role_id, $user_id); //Gaurav jain
    // }

...
class Reportoutput extends DBconnection implements OutputInterface
{
    ...
    {
        $this->conn = parent::mysqlConnection();
        //$this->campArr=$this->fetchCampaignArr();
    }

    public function setReportFields($reportObj, $config, $filerObj, $report_name, $current_month, $dwnl_reports_flag, $report_user_name,$table_name)
    {
        ...
        $this->filterval = $filerObj->getFilterCondition(); //:date("Y_m");
        ...
        if (!preg_match('/There are no/', $query)) {
            return ($this->dwnl_reports_flag) ? $this->downloadReportIntoCSV($query) : $this->fetchQueryRecords($query);
        } else {
            return $query;
        }
    ...
    public function fetchQueryRecords($query)
    {
        if ($this->report_name == "agent_break_report") {
        ...
        $this->reportquery = mysqli_query($this->conn, $query);
        if ($this->reportquery->num_rows) {
            ...
            while ($row = mysqli_fetch_row($this->reportquery)) {
                ...
                $retVal = implode(', ', $flattened);
                if($this->reportquery->num_rows == $this->initial+1){
                    print("{".$retVal."}");
                }else{
                    print("{".$retVal."},");
                }
            ...
            return "There are no records in this reports";
        }
    }
